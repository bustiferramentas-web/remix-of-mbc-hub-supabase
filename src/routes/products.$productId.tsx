...
export const Route = createFileRoute("/products/$productId")({
  component: () => <AuthGate><ProductDetail /></AuthGate>,
  notFoundComponent: () => (
...
function ProductDetail() {
  ...
  const expert = product ? experts.find((e) => e.id === product.expert_id) : undefined;
  ...
  const enrollTrend = months.map((m) => {
    ...
    const inMonth = productEnrollments.filter((e) => {
  ...
  if (!product || !expert || !palette) {
    return (
      <div className="p-10 max-w-3xl mx-auto">
        <div className="rounded-xl border border-border bg-white p-8 text-center">
          <p className="text-foreground font-semibold">Produto não encontrado.</p>
          <Link to="/products" className="text-primary underline mt-3 inline-block">Voltar para Produtos</Link>
        </div>
      </div>
    );
  }
            ...
            <div className="flex items-start gap-4 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 flex items-center gap-2 flex-wrap">
                  <span
                    className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold ${palette.chip}`}
                  >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <AlertCard
          ...
          rows={expiringSoon.slice(0, 8).map((e) => ({
            id: e.id,
            name: e.name,
            secondary: `vence em ${e.expiration_date}`,
...
function AlertCard({
  ...
  const toneCls = tone === "warn" ? "text-amber-600 bg-amber-50" : "text-red-600 bg-red-50";
  return (
    <div className="rounded-xl border border-border bg-white overflow-hidden">
      <div className="px-5 py-3 border-b border-border flex items-center gap-2">
        <span className={`inline-flex items-center justify-center h-6 w-6 rounded-md ${toneCls}`}>{icon}</span>
        <h3 className="font-semibold text-sm">{title}</h3>
        <span className="ml-auto text-xs font-semibold text-muted-foreground bg-secondary px-2 py-0.5 rounded-full">{count}</span>
      </div>
        ...
        {rows.length === 0 ? (
          ...
          rows.map((r) => (
            <Link
              ...
              to="/students/$id"
              params={{ id: r.id }}
              className="flex items-center justify-between gap-3 px-5 py-2.5 hover:bg-secondary/40 transition-colors"
            >
              <div className="min-w-0">
                <div className="text-sm font-medium truncate">{r.name}</div>
                <div className="text-xs text-muted-foreground truncate">{r.secondary}</div>
              </div>
              <span className={`shrink-0 text-xs font-semibold px-2 py-0.5 rounded-full ${toneCls}`}>{r.badge}</span>
            </Link>
          ))
        )}
      </div>
